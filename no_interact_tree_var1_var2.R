...
plot(bins[[var1]], bins[[var2]], type = "p",
...
REL_BA_species_number = paste0('REL_BA_', species_number)
plots_with_tree = which(fia_db$REL_BA_species_number > 0)
var1_var2_tree = cbind(tmpppt_df_binned[,var1][plots_with_tree], tmpppt_df_binned[,var1][plots_with_tree])
...
c = as.data.frame(cbind(tmpppt_df_binned[,var1], tmpppt_df_binned[,var1]))
...
#potential_area_with_interactions = 
...
#potential_area_no_interactions = 

# finding Relative Basal Area for 
plots_with_giant_sequoia = which(fia_db$species_number > 0)
total_basal_area = sum(fia_db$BASAL.AREA[plots_with_tree])  #total_basal_area = 
