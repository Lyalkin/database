...
setwd("E:/Keefe/") # change this on your computer
...
prediction_with261 = potential_with(c(1:19), clim_data)

prediction_without261 = potential_without(c(1:19), clim_data)
...
#require('graphics')
#mtext("Eastern Hemlock (Species 261) distribution", side=1)


#map('usa')
#points(x=fia_db$LON[which(prediction_with261 == T)], 
#       y=fia_db$LAT[which(prediction_with261 == T)])
#require('graphics')
#mtext("Model 'With interactions'", side=1)


#map('usa')
#points(x=fia_db$LON[which(prediction_without261 == T)], 
#       y=fia_db$LAT[which(prediction_without261 == T)])
#require('graphics')
...
pdf('Shapl_comparison_261.pdf', height=12, width=12)
par(mfrow=c(2,2))
